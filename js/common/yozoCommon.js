...
function getDocParamsValue(doc, key) {
  ...
  var l_Params = yozo.getPluginStorage().getItem(doc.getID());
  if (!l_Params) {
    return "";
  }
...
function setDocParamsValue(doc, key, value) {
  ...
  //把属性值整体再写回原来的文档ID中
  yozo.getPluginStorage().setItem(doc.getID(), JSON.stringify(l_objParams));
}
...
/******************************************************************************************/
//表单上传
function formUpload(uploadPath, saveType) {
  try {
    if (saveType == "")
      saveType = ".doc"
    var app = yozo.WApplication()
    ...
    var result = app.uploadFile(uploadPath, tmpSavePath);
    alert("上传成功");
...
function OnBtnSaveAsLocalFile() {
  ...
  // 设置YOZO文档对话框 2 FileDialogType:=msoFileDialogSaveAs
  var app = yozo.WApplication();
  ...
  if (l_ksoFileDialog.show() == -1) { // -1 代表确认按钮
    //alert("确认");
    // yozo.PluginStorage.setItem(constStrEnum.OADocUserSave, true); //设置保存为临时状态，在Save事件中避免OA禁止另存为对话框
    ...
    // yozo.ribbonUI.Invalidate(); //刷新Ribbon的状态
...
function OnBtnClearRevDoc() {
  ...
  //执行清稿操作前，给用户提示
  // if (!yozo.confirm("清稿操作将接受所有的修订内容，关闭修订显示。请确认执行清稿操作？")) {
  //     return;
  // }
  ...
  //yozo.ribbonUI.InvalidateControl("btnOpenRevision");
...
function OnAcceptAllRevisions() {
  ...
  if (l_Doc.getRevisions().getCount() >= 1) {
    // if (!yozo.confirm("目前有" + l_Doc.Revisions.Count + "个修订信息，是否全部接受？")) {
    //     return;
    // }
...
function OnRejectAllRevisions() {
  ...
  if (l_Doc.getRevisions().getCount() >= 1) {
    // if (!yozo.confirm("目前有" + l_Doc.Revisions.Count + "个修订信息，是否全部拒绝？")) {
    //     return;
    // }
...
function DoInsertPicToDoc() {
  ...
  l_shape.select();
  // l_shape.WrapFormat.Type = yozo.Enum&&yozo.Enum.wdWrapBehind||5; //图片的默认版式为浮于文字上方，可通过此设置图片环绕模式
}
 ...
 * }
 */
function OnInsertWaterMarkClicked() {
  var app = yozo.WApplication();
  var date = new Date();
  var waterContent = yozo.WApplication().getUserName() + " " + date.toLocaleDateString();
  app.getActiveWindow().getActivePane().getView().setSeekView(9);
  var shape = app.getSelection().getHeaderFooter().getShapes().addTextEffect(0, waterContent, "宋体", 16, 0, 0, 0, 0, null);
  shape.select(null);
  var shapeRange = app.getSelection().getShapeRange();
  shapeRange.getTextEffect().setNormalizedHeight(0);
  shapeRange.setName("ceshi");
  shapeRange.getLine().setVisible(0);
  shapeRange.getFill().setVisible(1);
  shapeRange.getFill().solid();
  shapeRange.getFill().getForeColor().setRGB(12632256);
  // shapeRange.getFill().setTransparency(0.1);//0.5;信创下面这个属性只支持整形
  shapeRange.setLockAspectRatio(1);
  //shapeRange.setHeight(130);//4.58 * 28.346;信创下面这个属性只支持整形
  //shapeRange.setWidth(796);//28.07 * 28.346;信创下面这个属性只支持整形
  shapeRange.setRotation(315); //控制旋转
  shapeRange.getWrapFormat().setAllowOverlap(1);
  shapeRange.getWrapFormat().setSide(3);
  shapeRange.getWrapFormat().setType(3);
  shapeRange.setRelativeHorizontalPosition(0);
  shapeRange.setRelativeVerticalPosition(0);
  shapeRange.setLeft(-999995);
  shapeRange.setTop(-999995);
  app.getActiveWindow().getActivePane().getView().setSeekView(0);

  var tmpSavePath = app.getTempPath() + new Date().getMilliseconds() + ".png";
  shape.saveAs(tmpSavePath);
  shape.delete();
  var doc = app.getActiveDocument();
  doc.getBackground().getFill().setTransparency(0);
  //doc.getBackground().getFill().setTextureTile(true);
  //doc.getBackground().getFill().userPicture(tmpSavePath);//大图填充
  doc.getBackground().getFill().userTextured(tmpSavePath);//平铺
  doc.getActiveWindow().getView().setDisplayBackgrounds(true);
}
//========================================================书签操作====================================================================
//插入书签
function addBookmark(bmkName) {
  try {
    var app = yozo.WApplication();
    var bookmarks = app.getActiveDocument().getBookmarks();

    if (!bookmarks.exists(bmkName)) {
      var range = app.getSelection().getRange();
      bookmarks.add(bmkName, range);
    } else {
      console.log("书签" + bmkName + "已存在");
    }
  } catch (e) {
    console.log(e);
  }
...
//获取书签值
function getBookMarkValue(bmkName) {
  try {
    var app = yozo.WApplication();
    var bookmarks = app.getActiveDocument().getBookmarks();
    var text;
    if (bookmarks.exists(bmkName)) {
      var bookmark = bookmarks.item(bmkName);
      var range = bookmark.getRange();
      text = range.getText();
      if (text.endsWith('\r\u0007')) {
        text = text.replace('\r\u0007', '');//表格中存在制表符
      }
      return text;
    } else {
      return "";
...
//书签赋值
function setBookmarkValue(bmkName, bmkValue) {
  try {
    var app = yozo.WApplication();
    var bookmarks = app.getActiveDocument().getBookmarks();
    ...
    if (bookmarks.exists(bmkName)) {
      ...
      bookmark.delete();
      range.select();
      range.setText(bmkValue);
      bookmarks.add(bmkName, range);
    } else {
      console.log("书签" + bmkName + "不存在");
    }
  } catch (e) {
    console.log(e);
  }
...
//在书签位置插入文档
function insertFileToBmk(bmkName, filePath) {
  var app = yozo.WApplication();
  var doc = app.getActiveDocument();
  var bookmarks = doc.getBookmarks();
  var bmk = null;
  try {
    bmk = bookmarks.item(bmkName);
    if (bmk) {
      var bmkRange = bmk.getRange();
      bmk.delete();
      bmk.select();
      var selection = app.getSelection();
      selection.insertFile(filePath, null, null, null, null);
      bookmarks.add(bmkName, bmkRange);
    }
  } catch (e) {
    console.log(e);
  }
...
function getBookmarkList() {
  ...
  for (var i = 1; i <= count; i++) {
    bookmarkList.push(bookmarks.item(i).getName());
  }
 ...
 * }
 */
function bookmarkExists(bookmarkName) {
  var app = yozo.WApplication();
  var doc = app.getActiveDocument();
  var bookmarks = doc.getBookmarks();

  return bookmarks.exists(bookmarkName);
}
function showBookmarks(isShow) {
  var doc = app.getActiveDocument();
  doc.getActiveWindow().getView().setShowBookmarks(isShow);
}

function selectBookmark(bmkName) {
  var app = yozo.WApplication();
  var doc = app.getActiveDocument();
  var bookmarks = doc.getBookmarks();
  var bmk = null;
...
function curToBookmark(bmkName) {
  var app = yozo.WApplication();
  var doc = app.getActiveDocument();
  var bookmarks = doc.getBookmarks();
  var bmk = null;
  try {
    bmk = bookmarks.item(bmkName);
    if (bmk) {
      var range = bmk.getRange();
      bmk.delete();
      range.setText(" ");
      bmk = bookmarks.add(bmkName, range);
      bmk.select();
      app.getSelection().move(2, -1);
    }
  } catch (e) {
    console.log(e);
  }
